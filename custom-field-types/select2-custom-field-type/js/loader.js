(function($){
    ...
    $( document ).ready( function(){
        ...
        if ( 'undefined' === typeof SpiderSEOAdminPageFrameworkSelect2FieldType ) {
            return;
        }
        debugLog( SpiderSEOAdminPageFrameworkSelect2FieldType );
    ...
    function initializeSelect2( oNode ) {
        ...
        if ( _aOptions[ 'search_callback' ] ) {
            ...
            _aOptions = $.extend(
                ...
                {
                    ajax: {
                        url: SpiderSEOAdminPageFrameworkSelect2FieldType.ajaxURL,
                        dataType: 'json',
                        ...
                        data: function (params) {
                            ...
                            return {
                                ...
                                doing_select2_ajax: SpiderSEOAdminPageFrameworkSelect2FieldType.nonce, // ensure it is called from here
        ...
        if ( _aOptions[ 'width' ] && 'auto' !== _aOptions[ 'width' ] ) {
            var _oFieldContainer = _oSelect2Target.closest( '.spider-seo-field-select2' );
            _oFieldContainer.css( 'width', _aOptions[ 'width' ] );
            _oFieldContainer.children( '.spider-seo-select-label' )
                .css( 'width', '100%' );
            _oFieldContainer.children( '.spider-seo-select-label' )
                .children( 'label' )
                .children( '.spider-seo-input-container' )
                .css( 'width', '100%' );
            _aOptions[ 'width' ] = '100%';
        }
        ...
        if ( _aOptions[ 'search_callback' ] ) {
             ...
             * Set initial values.
             */
            var _oInputForEncoded = _oSelect2Target.closest( '.spider-seo-field' )
    ...
    function toggleElementVisibility( subject ) {
        ...
        var _oAllSubjects    = _bGlobal
            ? $( _sSelectors )
            : $( subject ).closest( '.spider-seo-section' ).find( _sSelectors );
        _oAllSubjects.hide();

        // Get the revealing elements
        var _oElementToReveal       = _bGlobal
            ? $( _sTargetSelector )
            : $( subject ).closest( '.spider-seo-section' ).find( _sTargetSelector );
    ...
    function templateResultForIcon( state ) {
        var _oIconDefinition = $( state.element ).closest( '.spider-seo-field' ).find( '.custom-icon-definition[data-value=' + state.id + ']' );
    ...
    function debugLog( ...message ) {
        if ( ! parseInt( SpiderSEOAdminPageFrameworkSelect2FieldType.debugMode ) ) {
            return;
        }
