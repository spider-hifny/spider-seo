...
if ( ! class_exists( 'SpiderSEOPathCustomFieldType' ) ) :
include( dirname( __FILE__ ) . '/SpiderSEOPathCustomFieldType_Node.php' );
/**
 ...
 * @version     0.0.1
 */
class SpiderSEOPathCustomFieldType extends SpiderSEOAdminPageFramework_FieldType_image {
    ...
    protected function construct() {
        ...
        add_action( 'wp_ajax_apf_path_field_type-spider-seo' , '__return_empty_string' );
    }
        ...
        return array(
            ...
            array(
                ...
                'translation'    => array(
                    ...
                    'nonce'     => $_sNonce,
                    'label'     => array(
                        'selectPath' => __( 'Select Path', 'spider-seo' ),
                        'select'     => __( 'Select', 'spider-seo' ),
                    ),
                ),
                'translation_var' => 'SpiderSEOAdminPageFrameworkPathFieldType',
            ),
        );
    }
    ...
    public function getField( $aField ) {
        ...
        return $aField[ 'before_label' ]
            . "<div class='spider-seo-input-label-container spider-seo-input-container {$aField[ 'type' ]}-field'>" 
                . "<label for='{$aField[ 'input_id' ]}'>"
                    . $aField[ 'before_input' ]
                    . $this->getAOrB(
                        $aField[ 'label' ] && ! $aField[ 'repeatable' ],
                        "<span " . $this->getLabelContainerAttributes( $aField, 'spider-seo-input-label-string' ) . ">" 
                            . $aField[ 'label' ] 
                        . "</span>",                        
                        ''                        
                    )
         ...
         * @since       3.8.4
         * @since       3.9.0       Moved from `SpiderSEOPathCustomFieldType`.
         * @param       array       $aField
    ...
    protected function doOnFieldRegistration( $aFieldset ) {
        ...
        private function ___handleQuery( $aFieldset ) {
            ...
            if ( empty( $_POST[ 'spider-seo_path_field_type' ] ) ||  empty( $_POST[ 'nonce' ] ) ) {
                return;
            }
            ...
            $_sRelativePath = empty( $_POST[ 'id' ] ) || '#' === $_sNodeID
                ? '/'
                : trim( $_sNodeID, '\\/' );
            $_oTreeNode     = new SpiderSEOPathCustomFieldType_Node( $_sRelativePath, $_sRootDirPath, $_aPathOptions );
