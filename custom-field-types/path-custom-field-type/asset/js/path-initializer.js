...
(function ( $ ) {
  ...
  function initializeJSTree() {
    ...
    tb_show(
      SpiderSEOAdminPageFrameworkPathFieldType.label.selectPath,  // modal window title
    ...
    var _oButtonPanel  = $( '<div class="container-path-modal-select-button">'
      ...
      + '<div class="media-toolbar-primary search-form"><button type="button" class="button media-button button-primary button-small">' + SpiderSEOAdminPageFrameworkPathFieldType.label.select + '</button></div>'
      + '</div>' );
        ...
        core: {
          ...
          data: function ( node, cb ) {
            $.ajax( {
              type: 'post',
              dataType: 'json',
              url: SpiderSEOAdminPageFrameworkPathFieldType.ajaxURL,
              data: {
                id: node.id,
                action: 'apf_path_field_type-spider-seo',
                'spider-seo_path_field_type': 1,
                nonce: SpiderSEOAdminPageFrameworkPathFieldType.nonce,
