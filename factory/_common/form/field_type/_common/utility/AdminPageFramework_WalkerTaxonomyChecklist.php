...
class SpiderSEOAdminPageFramework_WalkerTaxonomyChecklist extends Walker_Category {
    public function start_el(&$sOutput, $oTerm, $iDepth=0, $aArgs=array(), $iCurrentObjectID=0)
    {
        ...
        $sOutput .= "\n" . "<li " . SpiderSEOAdminPageFramework_WPUtility::getAttributes($_aLiTagAttributes) . ">" . "<label for='{$_sID}' class='taxonomy-checklist-label'>" . $_sHiddenInputForUnchecked . "<input " . SpiderSEOAdminPageFramework_WPUtility::getAttributes($_aInputAttributes) . " />" . esc_html(apply_filters('the_category', $oTerm->name)) . $_sPostCount . "</label>";
    }
}
