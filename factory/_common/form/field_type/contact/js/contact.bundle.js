...
(function($){
  ...
  var apfContact = SpiderSEOAdminPageFrameworkContactFieldType;
  ...
  function sendEmail( event ) {
    ...
    // Check required
    var _requiredInputs = $( this ).closest( '.spider-seo-sections' )
    ...
    var _oSpinner = $('<img src="' + apfMain.spinnerURL + '" alt="' + apfMain.messages.loading + '" />' )
      .addClass( 'spider-seo-ajax-spinner' );
    $( this ).closest( '.spider-seo-field' ).find( '.result-placeholder' ).prepend( _oSpinner );
    var _resultIcon = $( this ).closest( '.spider-seo-field' ).find( '.result-placeholder .dashicons' );
    ...
    $.ajax( {
        ...
        success: function ( response ) {
          ...
          if ( response.result ) {
            _resultIcon.addClass( 'dashicons-yes-alt success spider-seo-form-tooltip' );
            _resultIcon.prepend( "<span class='spider-seo-form-tooltip-content'>" + response.message + "</span>" );
          } else {
            _resultIcon.addClass( 'dashicons-warning error spider-seo-form-tooltip' );
            _resultIcon.prepend( "<span class='spider-seo-form-tooltip-content'>" + response.message + "</span>" );
          }
        },
        error: function( response ) {
          ...
          _resultIcon.addClass( 'dashicons-warning error spider-seo-form-tooltip' );
          _resultIcon.prepend( "<span class='spider-seo-form-tooltip-content'>" + response.status + ' ' + response.statusText + "</span>" );
        },
        complete: function() {
          _oSpinner.remove();
          debugLog( 'Request done.' );
          _resultIcon[ 'spider-seo-form-tooltip' ]({
              autoClose: false,
              shown: true,
              oneOff: true,
              noArrow: true,
