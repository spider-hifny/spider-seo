...
class SpiderSEOAdminPageFramework_FieldType_textarea extends SpiderSEOAdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'spider-seo-field-type-textarea', 'src' => dirname(__FILE__) . '/js/textarea.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'spider-seo-script-form-main' ), 'translation_var' => 'SpiderSEOAdminPageFrameworkFieldTypeTextArea', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'label' => array(), ), ), );
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField)
    {
        ...
        $_sClassSelector = $_bIsArray ? 'spider-seo-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array( $this->getElementByLabel($aField['before_label'], $sKey, $aField[ 'label' ]), "<div class='spider-seo-input-label-container {$_sClassSelector}'>", "<label for='" . $aField[ 'attributes' ][ 'id' ] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField[ 'label' ]), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'spider-seo-input-label-string') . ">" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField[ 'label' ]), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField[ 'label' ]), );
        return implode('', $_aOutput);
    }
    private function _getEditor($aField)
    {
        ...
        ob_end_clean();
        return $_sContent . "<input type='hidden' class='spider-seo-textarea-data-input' data-tinymce-textarea='" . esc_attr($aField[ 'attributes' ][ 'id' ]) . "' />";
    }
}
