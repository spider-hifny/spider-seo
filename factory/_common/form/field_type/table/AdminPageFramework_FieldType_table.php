...
class SpiderSEOAdminPageFramework_FieldType_table extends SpiderSEOAdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'spider-seo-field-type-table', 'src' => dirname(__FILE__) . '/js/table.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'jquery-ui-accordion', 'spider-seo-script-form-main' ), 'translation_var' => 'SpiderSEOAdminPageFrameworkFieldTypeTable', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'label' => array(), ), ), );
    }
    ...
    private function ___getLabel($aField)
    {
        ...
        return "<div class='spider-seo-input-label-container'>" . "<label for='" . esc_attr($aField[ 'input_id' ]) . "'>" . "<span " . $this->getLabelContainerAttributes($aField, 'spider-seo-input-label-string') . ">" . $aField[ 'label' ] . "</span>" . "</label>" . "</div>";
    }
    private function ___getTable($aField)
    {
        ...
        $_sCaption = $aField[ 'caption' ] ? $aField[ 'caption' ] : __('Set the caption with the <code>caption</code> argument.', 'spider-seo');
