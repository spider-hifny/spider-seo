...
class SpiderSEOAdminPageFramework_Form_View___SectionCaption extends SpiderSEOAdminPageFramework_FrameworkUtility {
    ...
    private function _getCaption(array $aSectionset, $iSectionIndex, $aFieldsets, $aFieldErrors, $aFieldTypeDefinitions, $aCallbacks, $oMsg)
    {
        if (! $aSectionset[ 'description' ] && ! $aSectionset[ 'title' ]) {
            return "<caption class='spider-seo-section-caption' style='display:none;'></caption>";
        }
        $_oArgumentFormatter = new SpiderSEOAdminPageFramework_Form_Model___Format_CollapsibleSection($aSectionset[ 'collapsible' ], $aSectionset[ 'title' ], $aSectionset);
        $_abCollapsible = $_oArgumentFormatter->get();
        $_oCollapsibleSectionTitle = new SpiderSEOAdminPageFramework_Form_View___CollapsibleSectionTitle(array( 'title' => $this->getElement($_abCollapsible, 'title', $aSectionset[ 'title' ]), 'tag' => 'h3', 'section_index' => $iSectionIndex, 'collapsible' => $_abCollapsible, 'container_type' => 'section', 'sectionset' => $aSectionset, ), $aFieldsets, $this->aSavedData, $this->aFieldErrors, $aFieldTypeDefinitions, $oMsg, $aCallbacks);
        $_bShowTitle = empty($_abCollapsible) && ! $aSectionset[ 'section_tab_slug' ];
        return "<caption " . $this->getAttributes(array( 'class' => 'spider-seo-section-caption', 'data-section_tab' => $aSectionset[ 'section_tab_slug' ], )) . ">" . $_oCollapsibleSectionTitle->get() . $this->getAOrB($_bShowTitle, $this->_getCaptionTitle($aSectionset, $iSectionIndex, $aFieldsets, $aFieldTypeDefinitions), '') . $this->_getCaptionDescription($aSectionset, $aCallbacks[ 'section_head_output' ]) . $this->_getSectionError($aSectionset, $aFieldErrors) . "</caption>";
    }
    private function _getSectionError($aSectionset, $aFieldErrors)
    {
        ...
        return $_sSectionError ? "<div class='spider-seo-error'><span class='section-error'>* " . $_sSectionError . "</span></div>" : '';
    }
    private function _getCaptionTitle($aSectionset, $iSectionIndex, $aFieldsets, $aFieldTypeDefinitions)
    {
        $_oSectionTitle = new SpiderSEOAdminPageFramework_Form_View___SectionTitle(array( 'title' => $aSectionset[ 'title' ], 'tag' => 'h3', 'section_index' => $iSectionIndex, 'sectionset' => $aSectionset, ), $aFieldsets, $this->aSavedData, $this->aFieldErrors, $aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
        return "<div " . $this->getAttributes(array( 'class' => 'spider-seo-section-title', 'style' => $this->getAOrB($this->_shouldShowCaptionTitle($aSectionset, $iSectionIndex), '', 'display: none;'), )). ">" . $_oSectionTitle->get() . "</div>";
    }
    private function _getCaptionDescription($aSectionset, $hfSectionCallback)
    {
        ...
        $_oSectionDescription = new SpiderSEOAdminPageFramework_Form_View___Description($aSectionset[ 'description' ], 'spider-seo-section-description');
        return "<div class='spider-seo-section-description'>" . call_user_func_array($hfSectionCallback, array( $_oSectionDescription->get(), $aSectionset )) . "</div>";
    }
