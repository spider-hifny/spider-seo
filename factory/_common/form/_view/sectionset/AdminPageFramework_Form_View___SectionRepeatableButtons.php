...
class SpiderSEOAdminPageFramework_Form_View___SectionRepeatableButtons extends SpiderSEOAdminPageFramework_Form_Utility {
    public static function get($sContainerTagID, $iSectionCount, $asArguments, $oMsg)
    {
        ...
        $_oFormatter = new SpiderSEOAdminPageFramework_Form_Model___Format_RepeatableSection($asArguments, $oMsg);
    ...
    private static function ___getRepeatableSectionButtons($_aArguments, $oMsg, $sContainerTagID, $iSectionCount)
    {
        ...
        return "<div class='spider-seo-repeatable-section-buttons-outer-container'>" . "<div " . self::___getContainerAttributes($_aArguments, $oMsg) . ' >' . "<a " . self::___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount) . ">" . $_sIconRemove . "</a>" . "<a " . self::___getAddButtonAttributes($sContainerTagID, $oMsg, $_aArguments) . ">" . $_sIconAdd . "</a>" . "</div>" . "</div>" . SpiderSEOAdminPageFramework_Form_Utility::getModalForDisabledRepeatableElement('repeatable_section_disabled_' . $sContainerTagID, $_aArguments[ 'disabled' ]);
    }
    private static function ___getContainerAttributes(array $aArguments, $oMsg)
    {
        $_aAttributes = array( 'class' => self::getClassAttribute('spider-seo-repeatable-section-buttons', empty($aArguments[ 'disabled' ]) ? '' : 'disabled'), );
