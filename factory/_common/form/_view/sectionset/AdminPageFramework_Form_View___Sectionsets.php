...
class SpiderSEOAdminPageFramework_Form_View___Sectionsets extends SpiderSEOAdminPageFramework_Form_View___Section_Base {
    ...
    public function get()
    {
        $_oFormatSectionsetsByTab = new SpiderSEOAdminPageFramework_Form_View___Format_SectionsetsByTab($this->aStructure[ 'sectionsets' ], $this->aStructure[ 'fieldsets' ], $this->aArguments[ 'nested_depth' ]);
        $_aOutput = array();
        ...
        $_oDebugInfo = new SpiderSEOAdminPageFramework_Form_View___DebugInfo($this->aArguments[ 'structure_type' ], $this->aCallbacks, $this->oMsg);
        $_sOutput = implode(PHP_EOL, $_aOutput);
        $_sElementID = "spider-seo-sectionsets-" . uniqid();
        return $this->___getSpinnerOutput($_sOutput) . "<div id='{$_sElementID}' class='spider-seo-sectionsets spider-seo-form-js-on'>" . $_sOutput . $_oDebugInfo->get() . "</div>" ;
    }
    private function ___getSpinnerOutput($_sOutput)
    {
        if (trim($_sOutput)) {
            return "<div class='spider-seo-form-loading' style='display: none;'>" . $this->oMsg->get('loading') . "</div>";
        }
        return '';
    }
    private function ___getFormOutput(array $aSectionsets, array $aFieldsets, $sSectionTabSlug, $aCallbacks)
    {
        ...
        return $_sSectionSet ? "<div " . $this->getAttributes(array( 'class' => 'spider-seo-sectionset', 'id' => "sectionset-{$sSectionTabSlug}_" . md5(serialize($aSectionsets)), )) . ">" . $_sSectionSet . "</div>" : '';
    }
    ...
    {
        $_oArgumentFormater = new SpiderSEOAdminPageFramework_Form_Model___Format_CollapsibleSection($aSectionset[ 'collapsible' ], $aSectionset[ 'title' ], $aSectionset);
    ...
    private function ___getSectionsetTable($_aOutputs, $_sSectionsID, array $_aSection, array $aFieldsInSections)
    {
        ...
        $_oEachSectionArguments = new SpiderSEOAdminPageFramework_Form_Model___Format_EachSection($_aSection, null, array(), $_sSectionsID);
    ...
    private function ___getSubSections($_aOutputs, $_sSectionsID, $_aSection, $_aSubSections)
    {
        if (! empty($_aSection[ 'repeatable' ])) {
            $_aOutputs[ 'section_contents' ][] = SpiderSEOAdminPageFramework_Form_View___SectionRepeatableButtons::get($_sSectionsID, $_aOutputs[ 'count_subsections' ], $_aSection[ 'repeatable' ], $this->oMsg);
        ...
        foreach ($_aSubSections as $_iIndex => $_aFields) {
            $_oEachSectionArguments = new SpiderSEOAdminPageFramework_Form_Model___Format_EachSection($_aSection, $_iIndex, $_aSubSections, $_sSectionsID);
    ...
    private function ___getSectionTableWithTabList(array $_aOutputs, array $aSectionset, $aFieldsetsPerSection)
    {
        ...
        $_oSectionTable = new SpiderSEOAdminPageFramework_Form_View___Section($this->aArguments, $aSectionset, $this->aStructure, $aFieldsetsPerSection, $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->aCallbacks, $this->oMsg);
    ...
    private function ___getFormattedSectionsTablesOutput(array $aOutputs, $aSectionset, $sSectionsID, array $aCollapsible, $sSectionTabSlug)
    {
        ...
        $_oCollapsibleSectionTitle = new SpiderSEOAdminPageFramework_Form_View___CollapsibleSectionTitle(array( 'title' => $this->getElement($aCollapsible, 'title', ''), 'tag' => 'h3', 'section_index' => null, 'collapsible' => $aCollapsible, 'container_type' => 'sections', 'sectionset' => $aSectionset, ), array(), $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->oMsg, $this->aCallbacks);
        $_oSectionsTablesContainerAttributes = new SpiderSEOAdminPageFramework_Form_View___Attribute_SectionsTablesContainer($aSectionset, $sSectionsID, $sSectionTabSlug, $aCollapsible, $aOutputs[ 'count_subsections' ]);
    ...
    {
        return $sSectionTabSlug ? "<ul class='spider-seo-section-tabs nav-tab-wrapper'>" . implode(PHP_EOL, $aSectionTabList) . "</ul>" : '';
    }
    private function ___getTabList(array $aSection, array $aFields, $hfFieldCallback)
    {
        ...
        $_aTabAttributes = $aSection[ 'attributes' ][ 'tab' ] + array( 'class' => 'spider-seo-section-tab nav-tab', 'id' => "section_tab-{$_sSectionTagID}", 'style' => null );
        ...
        $_oSectionTitle = new SpiderSEOAdminPageFramework_Form_View___SectionTitle(array( 'title' => $aSection[ 'title' ], 'tag' => 'h4', 'section_index' => $iSectionIndex, 'sectionset' => $aSection, ), $aFields, $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->oMsg, $this->aCallbacks);
