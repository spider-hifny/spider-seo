...
abstract class SpiderSEOAdminPageFramework_Form_View___Fieldset_Base extends SpiderSEOAdminPageFramework_Form_Utility {
    ...
    {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset[ 'type' ], $aFieldTypeDefinitions) + SpiderSEOAdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    protected function _getRepeatableFieldButtons($sFieldsContainerID, $iFieldCount, $aSettings)
    {
        ...
        $_oFormatter = new SpiderSEOAdminPageFramework_Form_Model___Format_RepeatableField($_aSettings, $this->oMsg);
    ...
    {
        $_aContainerAttributes = array( 'class' => $this->getClassAttribute('spider-seo-repeatable-field-buttons', ! empty($aArguments[ 'disabled' ]) ? 'disabled' : ''), );
