...
class SpiderSEOAdminPageFramework_Form_View__Resource extends SpiderSEOAdminPageFramework_FrameworkUtility {
    public $oForm;
    ...
    private function ___setHooks()
    {
        ...
        new SpiderSEOAdminPageFramework_Form_View__Resource__Head($this->oForm, 'wp_head');
    }
    private function ___setAdminHooks()
    {
        ...
        new SpiderSEOAdminPageFramework_Form_View__Resource__Head($this->oForm, 'admin_head');
    }
    ...
    private function ___getFormattedEnqueueScript($asEnqueue)
    {
        ...
        $_aEnqueueItem = $this->getAsArray($asEnqueue) + array( 'handle_id' => 'spider-seo-script-form-' . $this->oForm->aArguments[ 'caller_id' ] . '_' . $_iCallCount, 'src' => null, 'dependencies' => null, 'version' => null, 'in_footer' => false, 'translation' => null, 'conditional' => null, 'translation_var' => null, );
    ...
    {
        static $_iCallCount = 1;
        $_aEnqueueItem = $this->getAsArray($asEnqueue) + array( 'handle_id' => 'spider-seo-style-form-' . $this->oForm->aArguments[ 'caller_id' ] . '_' . $_iCallCount, 'src' => null, 'dependencies' => null, 'version' => null, 'media' => null, 'conditional' => null, 'rtl' => null, 'suffix' => null, 'alt' => null, 'title' => null, );
        if (is_string($asEnqueue)) {
            $_aEnqueueItem[ 'src' ] = $asEnqueue;
        }
        $_aEnqueueItem[ 'src' ] = $this->___getSRCFormatted($_aEnqueueItem);
    ...
    public function _replyToAddStyle()
    {
        ...
        if ($_sCSSRules) {
            echo "<style type='text/css' id='internal-style-{$_sID}' class='spider-seo-form-style'>" . $_sCSSRules . "</style>";
        }
        $_sIECSSRules = $this->___getFormattedInternalStyles($this->oForm->getResources('internal_styles_ie'));
        if ($_sIECSSRules) {
            echo "<!--[if IE]><style type='text/css' id='internal-style-ie-{$_sID}' class='spider-seo-form-ie-style'>" . $_sIECSSRules . "</style><![endif]-->";
        }
    ...
    public function _replyToAddScript()
    {
        ...
        if ($_sScript) {
            $_sID = $this->sanitizeSlug(strtolower($this->oForm->aArguments[ 'caller_id' ]));
            echo "<script type='text/javascript' id='internal-script-{$_sID}' class='spider-seo-form-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
        }
