...
(function ( $ ) {
     ...
     */
    $.fn.selectAllSpiderSEOAdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
     ...
     */
    $.fn.deselectAllSpiderSEOAdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
            .find( 'input[type=checkbox]' )
            .prop( 'checked', false )
            .trigger( 'change' );   // 3.8.8+
...
( function( $ ) {
    ...
    var _removeSpiderSEOAdminPageFrameworkLoadingOutputs = function() {

        jQuery( '.spider-seo-form-loading' ).remove();
        jQuery( '.spider-seo-form-js-on' )
            .hide()
            .css( 'visibility', 'visible' )
            .fadeIn( 200 )
            .removeClass( '.spider-seo-form-js-on' );
    ...
    window.onerror = function(){
        ...
        _removeSpiderSEOAdminPageFrameworkLoadingOutputs();
...
(function ( $ ) {
    ...
    $.fn.aSpiderSEOAdminPageFrameworkInputOptions = {};

    $.fn.storeSpiderSEOAdminPageFrameworkInputOptions = function( sID, vOptions ) {
        ...
        $.fn.aSpiderSEOAdminPageFrameworkInputOptions[ sID ] = vOptions;
    };
    $.fn.getSpiderSEOAdminPageFrameworkInputOptions = function( sID ) {
        sID = sID.replace( /__\d+_/, '___' ); // remove the section index
        return ( 'undefined' === typeof $.fn.aSpiderSEOAdminPageFrameworkInputOptions[ sID ] )
            ? null
            : $.fn.aSpiderSEOAdminPageFrameworkInputOptions[ sID ];
    }
...
(function ( $ ) {
    ...
    // Callback containers.
    $.fn.aSpiderSEOAdminPageFrameworkAddRepeatableFieldCallbacks        = [];
    $.fn.aSpiderSEOAdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aSpiderSEOAdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aSpiderSEOAdminPageFrameworkSortedFieldsCallbacks              = [];
    $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aSpiderSEOAdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+

    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */
    $( document ).on( 'spider-seo_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aSpiderSEOAdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerSpiderSEOAdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     *
     * Unlike the `spider-seo_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).on( 'spider-seo_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aSpiderSEOAdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).on( 'spider-seo_stopped_sorting_sections', function( oEvent ){

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).on( 'spider-seo_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aSpiderSEOAdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aSpiderSEOAdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     */
    $( document ).on( 'spider-seo_saved_widget', function( event, oWidget ){
        $.each( $.fn.aSpiderSEOAdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 'function' !== typeof _hfCallback ) {
                return true;    // continue
     ...
     */
    $.fn.registerSpiderSEOAdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aSpiderSEOAdminPageFrameworkAddRepeatableFieldCallbacks.push(
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aSpiderSEOAdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );
        $.fn.aSpiderSEOAdminPageFrameworkRemoveRepeatableFieldCallbacks.push(
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aSpiderSEOAdminPageFrameworkSortedFieldsCallbacks.push(
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingFieldsCallbacks.push(
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aSpiderSEOAdminPageFrameworkAddedWidgetCallbacks.push(
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aSpiderSEOAdminPageFrameworkStoppedSortingSectionsCallbacks.push(
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );

    };
    /**
     * An alias of the `registerSpiderSEOAdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registerSpiderSEOAdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.spider-seo-fields.repeatable' ).each( function() {
            var _buttonModel = $( this ).siblings( '.repeatable-field-buttons-model' );
            if ( ! _buttonModel.length ) {
                return true;
            }
            var _aSettings           = _buttonModel.data();
            var _buttonContainer     = _buttonModel.children( '.spider-seo-repeatable-field-buttons' ).first();
            ...
            var _childFields = $( this ).find( '> .spider-seo-field.without-child-fields' );
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.spider-seo-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .spider-seo-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.spider-seo-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.spider-seo-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updateSpiderSEOAdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var SpiderSEOAdminPageFrameworkScriptFormMain
     */
    var translation = SpiderSEOAdminPageFrameworkScriptFormMain;
     ...
     */
    $.fn.updateSpiderSEOAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.spider-seo-repeatable-field-buttons' )
            .filter( function() {
                return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            });
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addSpiderSEOAdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removeSpiderSEOAdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.spider-seo-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.spider-seo-field' )
            .filter( function() {
                return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            })
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {
                $( '#' + _sFieldID ).addSpiderSEOAdminPageFrameworkRepeatableField( _sFieldID );
            }
        }
     ...
     */
    $.fn.addSpiderSEOAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {

        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.spider-seo-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.spider-seo-fields' );
        ...
        if ( ! _aOptions ) {
            var _nodeButtonContainer = nodeFieldContainer.find( '.spider-seo-repeatable-field-buttons' )
                .filter( function() {
                    return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.spider-seo-field' )
                                    .filter( function() {
                                        return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.spider-seo-repeatable-field-buttons' )
                                    .filter( function() {
                                        return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
         ...
         */
        nodeNewField.updateSpiderSEOAdminPageFrameworkRepeatableFields();
        ...
        nodeNewField.trigger(
            'spider-seo_added_repeatable_field',
            [
        ...
        $( nodeNewField ).find( '.spider-seo-field' ).addBack().trigger(
            'spider-seo_repeated_field',
            [
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.spider-seo-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {
                return $( this ).closest( '.spider-seo-fields' ).attr( 'id' ) === _sFieldsContainerID;
            })
        ...
        nodeFieldsContainer.children( '.spider-seo-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.spider-seo-field,.spider-seo-fields,.spider-seo-fieldset' ).incrementAttributes(
                [ 'id', 'data-field_tag_id_model', 'data-field_id' ],
                _iFieldCount,
                _sFieldTagIDModel
            );
            oElement.find( '.spider-seo-fields' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeSpiderSEOAdminPageFrameworkRepeatableField = function() {

        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.spider-seo-field' );
        var nodeFieldsContainer = $( this ).closest( '.spider-seo-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.spider-seo-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.spider-seo-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.spider-seo-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.spider-seo-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.spider-seo-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updateSpiderSEOAdminPageFrameworkRepeatableSections( _repeatOptions );
     ...
     */
    $.fn.updateSpiderSEOAdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.spider-seo-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {
        ...
        var _aOptions = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).on( 'click', function() {
            ...
            $( this ).addSpiderSEOAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // The Remove button behavior
        $( _oThis ).find( '.repeatable-section-remove-button' ).on( 'click', function() {
            $( this ).removeSpiderSEOAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // If the number of sections is less than the set minimum value, add sections.
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.spider-seo-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.spider-seo-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {
                $( '#' + _sSectionID ).addSpiderSEOAdminPageFrameworkRepeatableSection( _sSectionID );
            }
        }
     ...
     */
    $.fn.addSpiderSEOAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {

        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.spider-seo-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.spider-seo-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.spider-seo-section-tabs-contents' )
            .children( '.spider-seo-section-tabs' )
            .first();

        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );

        var _iFadein                = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        var _bPreserveValues        = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'preserve_values' ];

        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.spider-seo-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.spider-seo-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.spider-seo-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.spider-seo-section-title' ).not( '.spider-seo-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableSpiderSEOAdminPageFrameworkCollapsibleButton ){
            nodeNewSection.find( '.spider-seo-collapsible-sections-title, .spider-seo-collapsible-section-title' ).enableSpiderSEOAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.spider-seo-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updateSpiderSEOAdminPageFrameworkRepeatableFields();
            ...
            $( this ).trigger(
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.spider-seo-sections' )    // model container
                ]
            );
        ...
        nodeNewSection.updateSpiderSEOAdminPageFrameworkRepeatableSections();

        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.spider-seo-fields.sortable' ).each( function() {
            $( this ).enableSpiderSEOAdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.spider-seo-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.spider-seo-fieldrow, .spider-seo-fieldset, .spider-seo-fields, .spider-seo-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.spider-seo-fields' ).incrementAttribute(
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.spider-seo-fieldset' ).incrementAttribute(
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeSpiderSEOAdminPageFrameworkRepeatableSection = function() {

        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.spider-seo-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.spider-seo-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.spider-seo-section-tabs-contents' )
            .children( '.spider-seo-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.spider-seo-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aSpiderSEOAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.spider-seo-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.spider-seo-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() {
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.spider-seo-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.spider-seo-section-title' ).first().show();
                }
            }
        ...
        _oNextAllSections.each( function( _iIterationIndex ) {
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.spider-seo-field' ).each( function( iFieldIndex ) {
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.spider-seo-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.spider-seo-fields.sortable' ).each( function() {
            $( this ).enableSpiderSEOAdminPageFrameworkSortableFields();
        } );
    });

    $.fn.enableSpiderSEOAdminPageFrameworkSortableFields = function( sFieldsContainerID ) {
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.spider-seo-sections.sortable-section' ).each( function(){
            $( this ).enableSpiderSEOAdminPageFrameworkSortableSections();
        } );
    });

    $.fn.enableSpiderSEOAdminPageFrameworkSortableSections = function( sSectionsContainerID ) {
        ...
        var _bIsTabbed      = _oTarget.hasClass( 'spider-seo-section-tabs-contents' );
        var _bCollapsible   = 0 < _oTarget.children( '.spider-seo-section.is_subsection_collapsible' ).length;

        _oTarget        = _bIsTabbed
            ? _oTarget.find( 'ul.spider-seo-section-tabs' )
            : _oTarget;
        ...
        var _aSortableOptions = {
                items: _bIsTabbed
                    ? '> li:not( .disabled )'
                    : '.spider-seo-section:not( .disabled, .spider-seo-collapsible-toggle-all-button-container )',
                handle: _bCollapsible
                    ? '.spider-seo-section-caption'
                    : false,
                ...
                stop: function(e,ui) {
                    ...
                    // Callback the registered callback functions.
                    $( this ).trigger(
                        'spider-seo_stopped_sorting_sections',
                ...
                // cancel: '.spider-seo-section-description, .spider-seo-section-title'
        ...
        if ( ! _bIsTabbed ) {
            ...
            _oSortable.on( 'sortstop', function() {
                ...
                $( this ).find( 'caption > .spider-seo-section-title:not(.spider-seo-collapsible-sections-title,.spider-seo-collapsible-section-title)' ).first().show();
                $( this ).find( 'caption > .spider-seo-section-title:not(.spider-seo-collapsible-sections-title,.spider-seo-collapsible-section-title)' ).not( ':first' ).hide();
  ...
  function debugLog( ...message ) {
    if ( ! parseInt( SpiderSEOAdminPageFrameworkScriptFormMain.debugMode ) ) {
      return;
    }
...
(function($){
  ...
  $.widget( 'spider-seo.pointerTooltip', $.wp.pointer, {
   /**
 ...
 *
 * It provides the $[ 'spider-seo-tooltip' ]( options ) jQuery plugin method.
 * The name uses hyphens for the user's text domain to be replaced. So the compiled framework script will be entirely ported with without the keyword of `spider-seo`.
 *
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'spider-seo-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `spider-seo-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="spider-seo-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'spider-seo-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * ```
 *
 * If it the script is loaded, elements with the .spider-seo-from-tooltip class will be automatically parsed.
 * So not to call the `[ 'spider-seo-form-tooltip' ]()` method, just create an element with the selector
 * and it should automatically have a tooltip.
 *
 *
 * When the framework file is compiled, replace the keyword `spider-seo` with your text domain.
 */
(function($){

  // Initialize
  $( document ).ready( function() {
    $( '.spider-seo-form-tooltip' ).each( function() {
      $( this )[ 'spider-seo-form-tooltip' ]();
    } );
  } );

  $.fn[ 'spider-seo-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'spider-seo-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.spider-seo-form-tooltip-content' ).clone();
    ...
    var _options    = $.extend( true, {}, options, {
      ...
      position: {
        ...
        within: _offscreen ? _body : $( self ).closest( '.spider-seo-field, .spider-seo-fieldrow, .spider-seo-section' ),
      },
      buttons: function() {},
      close: function() {},
    }, options );
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.spider-seo-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.spider-seo-form-tooltip-balloon' ).on( 'mouseenter', function(){
    ...
    $( document ).ready( function() {
        ...
        $( document ).ajaxComplete( function( event, XMLHttpRequest, ajaxOptions ) {
            ...
            if( _aRequest.action && ( 'save-widget' === _aRequest.action ) ) {
                ...
                if ( $( _oWidget ).find( '.spider-seo-sectionset' ).length <= 0 ) {
                    return;
                }
