...
class SpiderSEOAdminPageFramework_AdminNotice extends SpiderSEOAdminPageFramework_FrameworkUtility {
    ...
    public function __construct($sNotice, array $aAttributes=array( 'class' => 'error' ), array $aCallbacks=array())
    {
        ...
        $this->aAttributes[ 'class' ] = $this->getClassAttribute($this->aAttributes[ 'class' ], 'spider-seo-settings-notice-message', 'spider-seo-settings-notice-container', 'notice', 'is-dismissible');
        $this->aCallbacks = $aCallbacks + $this->aCallbacks;
        new SpiderSEOAdminPageFramework_AdminNotice___Script;
        if (! $sNotice) {
            return;
        }
