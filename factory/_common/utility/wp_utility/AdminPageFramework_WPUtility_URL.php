...
class SpiderSEOAdminPageFramework_WPUtility_URL extends SpiderSEOAdminPageFramework_Utility {
    ...
    {
        $_sAdminURL = is_network_admin() ? network_admin_url(SpiderSEOAdminPageFramework_WPUtility_Page::getPageNow()) : admin_url(SpiderSEOAdminPageFramework_WPUtility_Page::getPageNow());
    ...
    public static function getSRCFromPath($sFilePath)
    {
        ...
        if (false !== strpos($sFilePath, $_sContentDirPath)) {
            $_sRelativePath = SpiderSEOAdminPageFramework_Utility::getRelativePath(WP_CONTENT_DIR, $sFilePath);
            $_sRelativePath = preg_replace("/^\.[\/\\\]/", '', $_sRelativePath, 1);
            return content_url($_sRelativePath);
        }
        $_sRelativePath = SpiderSEOAdminPageFramework_Utility::getRelativePath(ABSPATH, $sFilePath);
        $_sRelativePath = preg_replace("/^\.[\/\\\]/", '', $_sRelativePath, 1);
