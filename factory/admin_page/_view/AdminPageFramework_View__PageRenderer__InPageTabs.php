...
class SpiderSEOAdminPageFramework_View__PageRenderer__InPageTabs extends SpiderSEOAdminPageFramework_FrameworkUtility {
    public $oFactory;
    public $sPageSlug;
    public $sTag = 'h3';
    ...
    private function _getOutput($aInPageTabs, $sCurrentPageSlug, $sTag)
    {
        ...
        if (! $_aPage[ 'show_in_page_tabs' ]) {
            return $this->getElement($aInPageTabs, array( $_sCurrentTabSlug, 'title' )) ? "<{$_sTag} class='spider-seo-in-page-tab-title'>" . $aInPageTabs[ $_sCurrentTabSlug ][ 'title' ] . "</{$_sTag}>" : "";
        }
    ...
    {
        $_oTabBar = new SpiderSEOAdminPageFramework_TabNavigationBar($aTabs, $sActiveTab, $sTag, array( 'class' => 'in-page-tab', ), array( 'format' => array( $this, '_replyToFormatNavigationTabItem_InPageTab' ), 'arguments' => array( 'page_slug' => $sCurrentPageSlug, ), ));
        $_sTabBar = $_oTabBar->get();
        return $_sTabBar ? "<div class='spider-seo-in-page-tab'>" . $_sTabBar . "</div>" : '';
    }
    public function _replyToFormatNavigationTabItem_InPageTab(array $aTab, array $aStructure, array $aTabs, array $aArguments=array())
    {
        $_oFormatter = new SpiderSEOAdminPageFramework_Format_NavigationTab_InPageTab($aTab, $aStructure, $aTabs, $aArguments, $this->oFactory);
