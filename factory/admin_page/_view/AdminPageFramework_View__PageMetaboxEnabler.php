...
class SpiderSEOAdminPageFramework_View__PageMetaboxEnabler extends SpiderSEOAdminPageFramework_FrameworkUtility {
    public $oFactory;
    ...
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, array( 'aSpiderSEOAdminPageFramework', 'aMetaBoxForPagesClasses' ));
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, array( 'aSpiderSEOAdminPageFramework', 'bAddedMetaBoxScript' ), false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS[ 'aSpiderSEOAdminPageFramework' ][ 'bAddedMetaBoxScript' ] = true;
        $_sScript = <<<JAVASCRIPTS
jQuery(document).ready(function(){postboxes.add_postbox_toggles(pagenow)})
JAVASCRIPTS;
        echo '<script class="spider-seo-insert-metabox-script">' . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . '</script>';
    }
}
